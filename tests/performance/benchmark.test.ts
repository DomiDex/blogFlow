...
describe("Performance Benchmarks", () => {
  ...
  const testFormData: FormData = {
    ...
    metaDescription:
      "This article is used to measure the performance of our form processing pipeline",
  ...
  it("should handle concurrent processing efficiently", async () => {
    ...
    assert(
      duration < 2000,
      `Concurrent processing took ${duration.toFixed(2)}ms, exceeding 2000ms threshold`,
    );
  });
  ...
  it("should process large content efficiently", async () => {
    ...
    console.log(
      `Large content processing (${largeOps.length} paragraphs): ${duration.toFixed(2)}ms`,
    );
    ...
    assert(
      duration < 500,
      `Large content processing took ${duration.toFixed(2)}ms, exceeding 500ms threshold`,
    );
  });
  ...
  it("should maintain consistent slug generation performance", async () => {
    ...
    assert(
      avgDuration < 10,
      `Average slug generation time ${avgDuration.toFixed(2)}ms exceeds 10ms`,
    );

    // No single operation should take more than 50ms
    assert(
      maxDuration < 50,
      `Maximum slug generation time ${maxDuration.toFixed(2)}ms exceeds 50ms`,
    );
  });
